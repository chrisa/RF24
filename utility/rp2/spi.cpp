...
void PicoSPI::begin(spi_inst_t* hw_id)
{
    begin(hw_id, PICO_DEFAULT_SPI_SCK_PIN, PICO_DEFAULT_SPI_TX_PIN, PICO_DEFAULT_SPI_RX_PIN);
}

void PicoSPI::begin(spi_inst_t* hw_id, uint8_t _sck, uint8_t _tx, uint8_t _rx)
{
    _hw_id = hw_id;
...
uint8_t PicoSPI::transfer(uint8_t tx_)
{
    uint8_t recv = 0;
...
void PicoSPI::transfernb(const uint8_t* tbuf, uint8_t* rbuf, uint32_t len)
{
    spi_write_read_blocking(_hw_id, tbuf, rbuf, len);
}

void PicoSPI::transfern(const uint8_t* buf, uint32_t len)
{
    spi_write_blocking(_hw_id, buf, len);
}

void PicoSPI::beginTransaction(uint32_t _spi_speed)
{
